...
def determine_hand_type(cards: str) -> Enum:
    ...
    if max_count == FIVE_COUNT:
        ...
        return HandType.FULL_HOUSE if TWO_COUNT in card_map.values() else HandType.THREE_OF_A_KIND
    elif max_count == TWO_COUNT:
        return HandType.TWO_PAIR if list(card_map.values()).count(TWO_COUNT) == 2 else HandType.ONE_PAIR
    else:
        return HandType.HIGH_CARD
...
def parse_hands(lines: List[str], is_part_two: bool) -> List[CardHand]:
    return [CardHand(parts[0], parse_int(parts[1]), determine_hand_type_p2(parts[0]) if is_part_two else determine_hand_type(parts[0]))
...
def rank_hands(hands: List[CardHand]) -> List[CardHand]:
    return sorted(hands, key=lambda x: (card_hand_type_map[x._type], [card_rank_map[card] for card in x.cards]))
